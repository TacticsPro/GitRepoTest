...
public class GSTR_2B_Debit_Note_compare_Next_Oct_24
{
    ...
    public async Task Execute(Window mainWindow)
    {
        ...
        #region Pick-up Debit Note File
        var dialog = await ShowDialog.ShowMsgBox(
            title: "GSTR-2B Debit Note (Next Oct-24)",
            content: "This will merge all the Excel files (Debit Note Sheet) you select.\n\n Click OK to continue.",
        ...
        var listFileDialog = await ShowDialog.ShowMsgBox(
            ...
            content: "Pick the Excel file that contains the Creditors sheet.",
            "OK", "Cancel", 1, mainWindow);
        ...
        var inputSheetName = "Debit Note";
        var outputExcelFile = Path.Combine(subDir, "Debit_Note_2B.xlsx");
        var outputSheetName = "Debit Note";
    ...
    private async Task RunMacro(Window mainWindow, string outputExcelFile)
    {
        ...
        try
        {
            byte[] decryptedContentBytes = Transformation.GetTransformedFileContent("GSTR_2B_Debit_Note_compare_Next_Oct_24.dll");
            ...
            process.StartInfo.Arguments = $"\"{outputExcelFile}\" \"GSTR_2B_Debit_Note_compare_Next_Oct_24\"";
