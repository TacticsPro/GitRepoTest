...
public class Sales_Adjust_Self
{
    ...
    public async Task Execute(Window mainWindow)
    {
        ...
        var outputExcelFile = Path.Combine(subDir, "Sales(Adjusted_self).xlsx");
    ...
    private async Task RunMacro(Window mainWindow, string outputExcelFile)
    {
        ...
        try
        {
            byte[] decryptedContentBytes = Transformation.GetTransformedFileContent("Sales_Adjust_Self.dll");
            ...
            process.StartInfo.Arguments = $"\"{outputExcelFile}\" \"Sales_Adjust_Self\"";
