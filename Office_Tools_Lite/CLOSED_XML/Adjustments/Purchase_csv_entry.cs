...
public class Purchase_csv_entry
{
    ...
    public async Task Execute(Window mainWindow)
    {
        ...
        var outputExcelFile = Path.Combine(subDir, "Purchase_2A_entry.xlsx");
        ...
        bool appendSuccess = await CSV_Appender.AppendCsvToExcel(inputcsvFiles, outputExcelFile, outputSheetName);
    ...
    private async Task RunMacro(Window mainWindow, string outputExcelFile)
    {
        ...
        try
        {
            byte[] decryptedContentBytes = Transformation.GetTransformedFileContent("Purchase_csv_entry.dll");
            ...
            process.StartInfo.Arguments = $"\"{outputExcelFile}\" \"Purchase_csv_entry\"";
