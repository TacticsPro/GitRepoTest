...
public class GSTR_2B_Credit_Note_compare_Next_Oct_24
{
    ...
    public async Task Execute(Window mainWindow)
    {
        ...
        var dialog = await ShowDialog.ShowMsgBox(
            "GSTR-2B Credit Note (Next Oct-24)",
        ...
        var outputExcelFile = Path.Combine(subDir, "Credit_Note_2B.xlsx");
    ...
    private async Task RunMacro(Window mainWindow, string outputExcelFile)
    {
        ...
        try
        {
            byte[] decryptedContentBytes = Transformation.GetTransformedFileContent("GSTR_2B_Credit_Note_compare_Next_Oct_24.dll");
            ...
            process.StartInfo.Arguments = $"\"{outputExcelFile}\" \"GSTR_2B_Credit_Note_compare_Next_Oct_24\"";
