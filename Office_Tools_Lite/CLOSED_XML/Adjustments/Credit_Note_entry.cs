...
public class Credit_Note_entry
{
    ...
    public async Task Execute(Window mainWindow)
    {
        ...
        var outputExcelFile = Path.Combine(subDir, "Credit_Note_Enrty.xlsx");
    ...
    private async Task RunMacro(Window mainWindow, string outputExcelFile)
    {
        ...
        try
        {
            byte[] decryptedContentBytes = Transformation.GetTransformedFileContent("Credit_Note_entry.dll");
            ...
            process.StartInfo.Arguments = $"\"{outputExcelFile}\" \"Credit_Note_entry\"";
