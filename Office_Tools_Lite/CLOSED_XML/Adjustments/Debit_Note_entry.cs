...
public class Debit_Note_entry
{
    ...
    public async Task Execute(Window mainWindow)
    {
        ...
        var outputExcelFile = Path.Combine(subDir, "Debit_Note_entry.xlsx");
    ...
    private async Task RunMacro(Window mainWindow, string outputExcelFile)
    {
        ...
        try
        {
            byte[] decryptedContentBytes = Transformation.GetTransformedFileContent("Debit_Note_entry.dll");
            ...
            process.StartInfo.Arguments = $"\"{outputExcelFile}\" \"Debit_Note_entry\"";
