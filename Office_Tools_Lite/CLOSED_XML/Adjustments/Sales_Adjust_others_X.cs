...
public class Sales_Adjust_others_X
{
    ...
    public async Task Execute(Window mainWindow)
    {
        ...
        #region Pick-up Sales File
        var dialog = await ShowDialog.ShowMsgBox(
            "Sales Adjustments Others_X",
        ...
        #region Pick-up Debtors File
        var listFileDialog = await ShowDialog.ShowMsgBox(
            "Debtors List",
            "Pick the Excel file that contains the Debtors sheet.",
            "OK", "Cancel", 1, mainWindow);
        ...
        if (listFile == null)
        {
            await ShowDialog.ShowMsgBox("Warning", "No debtors file selected", "OK", null, 1, App.MainWindow);
            return;
        }
        #endregion
        ...
        var inputSheetName = "Sales";
        var outputExcelFile = Path.Combine(subDir, "Sales(Adjusted_others).xlsx");
        var outputSheetName = "Sales";
        var FirstRowContent = "Date";
        var listFilePath = listFile.Path;
        var sourceSheetName = "Debtors";
        var targetSheetName = "Debtors";
    ...
    private async Task RunMacro(Window mainWindow, string outputExcelFile)
    {
        ...
        try
        {
            byte[] decryptedContentBytes = Transformation.GetTransformedFileContent("Sales_Adjust_others_X.dll");
            ...
            process.StartInfo.Arguments = $"\"{outputExcelFile}\" \"Sales_Adjust_others_X\"";
