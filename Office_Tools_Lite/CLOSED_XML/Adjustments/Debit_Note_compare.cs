...
public class Debit_Note_compare
{
    ...
    public async Task Execute(Window mainWindow)
    {
        ...
        var dialog = await ShowDialog.ShowMsgBox(
            "Debit Note (Compare)",
            "This will merge all the Excel files (Debit Note Sheet) you select.\n\n Click OK to continue.",
        ...
        var outputExcelFile = Path.Combine(subDir, "Debit_Note_compare.xlsx");
    ...
    private async Task RunMacro(Window mainWindow, string outputExcelFile)
    {
        ...
        try
        {
            byte[] decryptedContentBytes = Transformation.GetTransformedFileContent("Debit_Note_compare.dll");
            ...
            process.StartInfo.Arguments = $"\"{outputExcelFile}\" \"Debit_Note_compare\"";
