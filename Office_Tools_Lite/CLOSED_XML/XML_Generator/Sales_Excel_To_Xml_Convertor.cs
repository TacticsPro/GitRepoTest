...
public class Sales_Excel_To_Xml_Converter
{
    ...
    public async Task Execute(Window mainWindow)
    {
        ...
        if (file != null)
        {
            string FinancialYear = await ShowDialog.ShowInputDialog("Sales XML Converter", mainWindow);
            ...
            try
            {
                ...
                string outputPath = Path.Combine(Path.GetDirectoryName(excelPath), "Sales_Vouchers.xml");
                xml.Save(outputPath);
    ...
    private XElement CreateVoucherElement(DataRow row, DataTable table)
    {
        ...
        string vchType = table.Columns.Contains("Voucher Type") && !string.IsNullOrWhiteSpace(row["Voucher Type"]?.ToString()) ? row["Voucher Type"].ToString() : "Sales";
            ...
            var ledgerKeysForGross = new List<string>
                {
                    ...
                    "CESS", "Labour", "Room Rent @ 12%", "Room Rent @ 5%", "Round Off"
                };
