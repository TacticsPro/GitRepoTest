...
public class Purchase_Excel_To_Xml_Converter
{
    ...
    public async Task Execute(Window mainWindow)
    {
        ...
        if (file != null)
        {
            string FinancialYear = await ShowDialog.ShowInputDialog("Purchase XML Converter", mainWindow);
            ...
            try
            {
                ...
                string outputPath = Path.Combine(Path.GetDirectoryName(excelPath), "Purchase_Vouchers.xml");
                xml.Save(outputPath);
    ...
    private XElement CreateVoucherElement(DataRow row, DataTable table)
    {
        ...
        string vchType = table.Columns.Contains("Voucher Type") && !string.IsNullOrWhiteSpace(row["Voucher Type"]?.ToString()) ? row["Voucher Type"].ToString() : "Purchase";
