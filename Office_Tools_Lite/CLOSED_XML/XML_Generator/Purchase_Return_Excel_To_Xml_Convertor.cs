...
public class Purchase_Return_Excel_To_Xml_Convertor
{
    ...
    public async Task Execute(Window mainWindow)
    {
        ...
        if (file != null)
        {
            string FinancialYear = await ShowDialog.ShowInputDialog("Purchase Return XML Converter", mainWindow);
            ...
            try
            {
                ...
                if (!ValidateExcelData(_table, year1, year2, FYmonth, out emptyDates, out invalidDateFormats, out incorrectYearDates, out monthMismatchErrors, out noBillNos, out noNameBills, out noValueBills, out negativeBills, out grossMismatchBills))
                {
                    await ShowDialog.ShowMsgBox("Date Validation Error", "There are few errors in Excel data...!", "OK", null, 1, mainWindow);
                    await HighlightErrorsInReadyDataSheetAsync(excelPath, emptyDates, invalidDateFormats, incorrectYearDates, monthMismatchErrors, noBillNos, noNameBills, noValueBills, negativeBills, grossMismatchBills);
                    return;
                }

                XDocument xml = BuildTallyXml(_table, YN, FY);
                string outputPath = Path.Combine(Path.GetDirectoryName(excelPath), "Purchase_Return_Vouchers.xml");
                xml.Save(outputPath);
    ...
    private XElement CreateVoucherElement(DataRow row, DataTable table)
    {
        ...
        string vchType = table.Columns.Contains("Voucher Type") && !string.IsNullOrWhiteSpace(row["Voucher Type"]?.ToString()) ? row["Voucher Type"].ToString() : "Debit Note";
        ...
        {
            { "Non GST", "Purchase Non GST Exempted" + $"{type}" },
            { "RD", "Purchase from RD" + $"{type}" },
            { "Taxable 0%", "Purchase GST @ 0%" + $"{type}" },
            { "Taxable 5%", "Purchase GST @ 5%" + $"{type}" },
            { "SGST 2.5%", "Input SGST @ 2.5%" + $"{type}" },
            { "CGST 2.5%", "Input CGST @ 2.5%" + $"{type}" },
            { "Taxable 12%", "Purchase GST @ 12%" + $"{type}" },
            { "SGST 6%", "Input SGST @ 6%" + $"{type}" },
            { "CGST 6%", "Input CGST @ 6%" + $"{type}" },
            { "Taxable 18%", "Purchase GST @ 18%" + $"{type}" },
            { "SGST 9%", "Input SGST @ 9%" + $"{type}" },
            { "CGST 9%", "Input CGST @ 9%" + $"{type}" },
            { "Taxable 28%", "Purchase GST @ 28%" + $"{type}" },
            { "SGST 14%", "Input SGST @ 14%" + $"{type}" },
            { "CGST 14%", "Input CGST @ 14%" + $"{type}" },
            {"Inter State 0%", "Interstate Purchase GST @ 0%" + $"{type}" },
            {"Inter State 5%", "Interstate Purchase GST @ 5%" + $"{type}" },
            {"IGST 5%", "Input IGST @ 5%" + $"{type}" },
            {"Inter State 12%", "Interstate Purchase GST @ 12%" + $"{type}" },
            {"IGST 12%", "Input IGST @ 12%" + $"{type}" },
            {"Inter State 18%", "Interstate Purchase GST @ 18%" + $"{type}" },
            {"IGST 18%", "Input IGST @ 18%" + $"{type}" },
            {"Inter State 28%", "Interstate Purchase GST @ 28%" + $"{type}" },
            {"IGST 28%", "Input IGST @ 28%" + $"{type}" },
            {"CESS", "Input Cess @ 12%" + $"{type}" },
        ...
        foreach (var kvp in ledgerMapping)
        {
            ...
            if (table.Columns.Contains(colName))
            {
                ...
                if (decimal.TryParse(amount, out decimal amt) && amt != 0)
                {
                    ...
                    string finalAmount = amt.ToString("F2"); // No sign change
    ...
    private bool ValidateExcelData(DataTable table, int year1, int year2, string FYmonth, out List<(int Row, string Date)> emptyDates, out List<(int Row, string Date)> invalidDateFormats, out List<(int Row, string Date)> incorrectYearDates, out List<(int Row, string Date)> monthMismatchErrors,
        out List<int> noBillNos, out List<int> noNameBills, out List<int> noValueBills, out List<int> negativeBills, out List<int> grossMismatchBills)
    {
        ...
        string[] taxableCols = {
                "Non GST", "RD", "Taxable 0%", "Taxable 5%", "Taxable 12%",
        ...
        foreach (DataRow row in table.Rows)
        {
            ...
            var ledgerKeysForGross = new List<string>
                {
                    "Non GST", "RD", "Taxable 0%", "Taxable 5%",
                    "SGST 2.5%", "CGST 2.5%",
    ...
    private async Task HighlightErrorsInReadyDataSheetAsync(string excelPath, List<(int Row, string Date)> emptyDates, List<(int Row, string Date)> invalidDateFormats,
                    ...
                    List<int> negativeBills, List<int> grossMismatchBills)
    {
        try
        {
